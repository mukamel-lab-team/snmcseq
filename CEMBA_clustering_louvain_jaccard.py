...
# def gen_knn_graph_bipartite(X, Y, k, option='spearman_corr'):
#     """
#     Generate knn graph from data matrix (n_obs, n_features)
#     Edges are weighted by jaccard index between 2 nodes

#     X: array like (n_obs, n_features) 
#     Y: array like (n_obs, n_features) 
#     k: int, number of nearest neighbors
#     option: directed or indirected 
#     """

#     assert X.shape[1] == Y.shape[1]
#     if option == 'spearman_corr':
#         # get data rankings 
#         X = pd.DataFrame(X).rank(axis=1, pct=True).values
#         Y = pd.DataFrame(Y).rank(axis=1, pct=True).values

#         # spearman corr 
#         Z = np.corrcoef(np.concatenate(X, Y))
#         XY = Z[:len(X.shape[0]), len(X.shape[0]):] 

#         # knn on XY (nx_obs, ny_obs)
