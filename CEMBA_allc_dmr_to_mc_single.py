...
def allc_dmr_to_mc_single_worker(allc_files, dmr_files, output_prefixes, use_dmrs='True'):
    """
    Args:
        - allc_files (allc_table 'allc_.tsv' or 'allc_.tsv.gz' or 'allc_.tsv.bgz')
        - dmr_files (bed file '.bed')
        - output_prefixes (output will be '{}_{}.tsv'.format(output_prefixes, chrom))
    """
    # allc to mc
    for i, (allc_file, dmr_file, output_prefix) in enumerate(zip(allc_files, dmr_files, output_prefixes)):
...
def allc_dmr_to_mc_single(allc_files, dmr_files, output_dir, use_dmrs='True'):
    """
    Args:
        - allc_files (allc_table 'allc_.tsv' or 'allc_.tsv.gz' or 'allc_.tsv.bgz')
        - dmr_files (bed file '.bed')
    ...
    output_prefixes = [os.path.join(
        ...
        .replace('allc', 'mc_single')
        .split('.')[0]
        )
        for allc_file in allc_files]
    
    # make sure they are matched
    assert len(allc_files) == len(dmr_files)
    logging.info(np.array(list(zip(allc_files, dmr_files, output_prefixes))))
    
    # work
    allc_dmr_to_mc_single_worker(allc_files, dmr_files, output_prefixes, use_dmrs=use_dmrs)
