...
def bin_allc_worker(allc_file, output_file,
    ...
    try:
        ...
        sp.run("mv {}.gz {}.gz".format(output_file, output_file), shell=True)
    except:
        sp.call("bgzip -f {}".format(output_file), shell=True)
        sp.call("mv {}.gz {}.gz".format(output_file, output_file), shell=True)
...
def create_parser():
    ...
    parser.add_argument("-i", "--input", help="allc file path", required=True)
...
if __name__ == '__main__':
    ...
    ti = time.time()
    bin_allc_worker(args.input, args.output,
