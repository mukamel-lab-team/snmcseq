...
def plot_tsne_label(input_fname, meta_fname, label_col, merge_col='Sample', 
	...
	# order are arranged for exci-inhi-glia plot 11/1/2017
	colors = ['C0', 'C2', 'C1', 'C3', 'C4', 'C5', 'C6', 'C8', 'C9']
	for i, (label, df_sub) in enumerate(df_merged.fillna('unlabelled').groupby(label_col)):
		if label == 'unlabelled':
			ax.plot(df_sub['tsne_x'], df_sub['tsne_y'], 'o', color='gray',
					label=label, markersize=1)	
		else:
			ax.plot(df_sub['tsne_x'], df_sub['tsne_y'], 'o', color=colors[i%len(colors)], 
					label=label, markersize=1)
