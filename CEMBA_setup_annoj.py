...
def gen_php_template(track_name, template_string, database=DATABASE_ANNOJ):
    """
    """
    return template_string.format(database, track_name, track_name, track_name)
...
def setup_annoj_main(ens, cluster_type, title_name=os.path.basename(PATH_PROJECT)):
    ...
    # allc_files = sorted(glob.glob(os.path.join(PATH_ENSEMBLES, ens, 'allc_merged/{}/allc_merged_mCG_{}_*_{}.tsv'.format(cluster_type, cluster_type, ens))))
    # # for allc_file in sorted(glob.glob(os.path.join('allc_merged/{}/allc_merged_{}_{}.tsv'.format(cluster_type, )))):
    ...
    #     cmd = '/cndd/fangming/lab_scripts/load_mysql_mc {} > ./annoj_1.log'.format(DATABASE_ANNOJ)
    #     sp.call(cmd, shell=True)
    ...
    # n_clusters = len(glob.glob(os.path.join(ens_path, 'allc_merged/{}/allc_merged_mCG_{}_*_{}.tsv'.format(cluster_type, cluster_type, ens))))
    # cluster_numbers = [int(os.path.basename(file)[len('allc_merged_mCG_{}_'.format(cluster_type)):-len('_{}.tsv'.format(ens))])
    #           for file in glob.glob(os.path.join(ens_path, 'allc_merged/{}/allc_merged_mCG_{}_*_{}.tsv'.format(cluster_type, cluster_type, ens)))]
    ...
    #         df_hypo.apply(lambda x: ('merged_mCG_{}_{}_{}_mergestrands'.format(cluster_type, i, ens) in x))] # mergestrands
    #     output = os.path.join(output_folder_dmr, 'dmr_hypo_allc_merged_mCG_{}_clst{}.bed'.format(cluster_type, i))
    ...
    track_names_mc = ['mc_merged_mCG_{}_clst{}_{}'.format(cluster_type, num, ens) for num in cluster_numbers]
    track_names_dmr = ['dmr_hypo_mCG_{}_clst{}_{}'.format(cluster_type, num, ens) for num in cluster_numbers]


    output_folder_php = PATH_PROJECT + '/browsers/php_mc_{}'.format(ens)
    ...
    output_folder_php = PATH_PROJECT + '/browsers/php_dmrs_{}'.format(ens)
    if not os.path.isdir(output_folder_php):
        os.makedirs(output_folder_php)
        logging.info('Created directory: {}'.format(output_folder_php))
    ...
    track_vis_names_mc = track_names_mc
    track_vis_names_dmr = track_names_dmr 

    title = '{} ({})'.format(title_name, ens)
    ...
    # gen_index
    index_html = gen_index_html(TEMPLATE_INDEX_HTML_HUMAN, title, mc_configs, dmr_configs, active_configs)

    # output 
    output_html = PATH_PROJECT + '/browsers/index_{}.html'.format(ens)
...
if __name__ == '__main__':
    ...
    ens = 'Ens0'
    cluster_type = 'mCH_npc50_k30_merged' # 'cluster_' prefix
    setup_annoj_main(ens, cluster_type)
